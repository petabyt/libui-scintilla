...
#define uiScintillaSignature FOURCC('S', 'c', 'i', 'n')
...
uintptr_t uiScintillaSendMessage(uiScintilla *s, uint32_t code, uintptr_t w, uintptr_t l) {
	return scintilla_object_send_message(s->sci, code, w, (gintptr)l);
}

void uiScintillaGetRange(uiScintilla *s, unsigned int start, unsigned int end, char *text) {
	struct Scintilla::TextRange tr = {0};
	tr.chrg.cpMin = start;
	tr.chrg.cpMax = end;
...
void uiScintillaSetText(uiScintilla *s, const char *text, unsigned int len) {
	scintilla_object_send_message(s->sci, SCI_SETTEXT, len, reinterpret_cast<gintptr>(text));
}
